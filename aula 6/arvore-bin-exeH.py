...
class BinaryTree:
    ...
    def postorder_traversal(self, node=None):
        ...
        #tentei mas n consegui pensar em uma lógica que funcionasse.
    ...
    def height(self, node=None):
        ...
        if node.left:
            hleft = 1 + self.height(node.left)
        if node.right:
            hright = 1 + self.height(node.right)
        
        # esse max eu tive que pesquisar. Não sabia da existencia disso.
